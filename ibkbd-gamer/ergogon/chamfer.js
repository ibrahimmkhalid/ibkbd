...
lines.forEach((line) => {
  if (line.includes("CAG.circle")) {
    ...
    let x = xy[0];
    let y = xy[1];
    let cyl = `new CSG.cylinder({start:[${x},${y},${2 - screwtop_height}],end:[${x},${y},2],radiusStart:${screw_radius},radiusEnd:${screwtop_radius}})`;
    cylinders.push(cyl);
  }
});
...
fs.copyFileSync(
  "./output/cases/bottom_plate.jscad",
  outpath + "bottom_plate.jscad",
);
